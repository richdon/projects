...
def load_neos(neo_csv_path):
    ...
    def neo_objects(neo_lst):
        ...
        for neo in neo_lst:
            return NearEarthObject(designation = neo['pdes'], name = neo['name'],
                                   diameter = neo['diameter'], hazardous = ['pha'])
    
    neo_obj_lst = []   
    
    for neo in range(len(neo_lst)):
        neo_obj_lst.append(neo_objects(neo_lst))
...
def load_approaches(cad_json_path):
    ...
    def ca_objects(approach_lst):
        
        for ca in approach_lst:
            return CloseApproach(des = ca['des'], time = ca['cd'],
                                 distance = ca['dist'], velocity = ca['v_rel'])
   
    ca_obj_lst = []
    
    for ca in range(len(approach_lst)):
        ca_obj_lst.append(ca_objects(approach_lst))
